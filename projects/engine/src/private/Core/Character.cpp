...
namespace MAD
{
	...
	{
		GetFirstComponentByType<CCameraComponent>().lock()->TEMPInitializeCameraInstance(ConvertToRadians(60.0f), 3.0f, 100000.0f, Vector3(0, 150.0f, 100.0f), Quaternion::Identity);
		...
		dir.Normalize();
		GetFirstComponentByType<CDirectionalLightComponent>().lock()->TEMPInitializeDirectionalLight(dir, Color(1.0f, 1.0f, 1.0f, 1.0f), 0.4f);

		GetFirstComponentByType<CPointLightComponent>().lock()->TEMPInitializePointLight(Vector3(0.0f, 0.0f, 0.0f), Color(1.0f, 1.0f, 1.0f, 1.0f), 2.0f, 150.0f, 600.0f);
	}
}
