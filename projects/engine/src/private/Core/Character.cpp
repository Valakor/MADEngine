...
namespace MAD
{
	...
	{
		GetFirstComponentByType<CCameraComponent>().lock()->TEMPInitializeCameraInstance(ConvertToRadians(60.0f), 3.0f, 100000.0f, Matrix::Identity);
		...
		dir.Normalize();
		GetFirstComponentByType<CDirectionalLightComponent>().lock()->TEMPInitializeDirectionalLight(dir, Color(1.0f, 1.0f, 1.0f, 1.0f), 2.0f);
	}
}
